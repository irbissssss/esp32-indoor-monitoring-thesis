...
void shutDownRadio() {
  // Only deinit ESP-NOW if it was actually initialized.
  // This prevents Guru Meditation crashes if sensor setup failed before ESP-NOW started.
  if (espNowReady) {
    esp_now_deinit();
    espNowReady = false;
  }
...
void goToSleep() {
  ...
  Serial.println("If you need to upload new code, click Upload now or use BOOT mode.");
  Serial.flush();
...
bool setupSensor() {
  ...
  // Slower I2C is more stable with jumper wires / breadboard prototypes
  Wire.setClock(50000);
  ...
  // Reset sensor before first reading
  sht31.reset();
  delay(500);
...
bool setupEspNow() {
  ...
  memcpy(peerInfo.peer_addr, mainHubMac, 6);
...
bool readSht31WithRetry(float &temperature, float &humidity) {
...
void setup() {
  ...
  Serial.println();
  Serial.println("=================================");
  Serial.println("Nordic Home Sensor Node 1");
  ...
  // Give ESP-NOW time to actually transmit before sleeping
  delay(1000);
